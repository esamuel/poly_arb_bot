...
def check():
    if not PROXY:
        print("No PROXY_ADDRESS env")
        return

    ctr = w3.eth.contract(address=Web3.to_checksum_address(USDC_NATIVE), abi=ABI)
    allow = ctr.functions.allowance(Web3.to_checksum_address(PROXY), Web3.to_checksum_address(SPENDER)).call()
    print(f"Native USDC Allowance for Proxy -> Spender: {allow}")
